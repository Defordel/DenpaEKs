...
class ChartingState extends MusicBeatState {
	...
	public static var GRID_SIZE(get, null):Int = 40; // one does not require large memory mhm mhm
	public static function get_GRID_SIZE():Int {
		var m:Int = PlayState.mania;
		if (m < 9) return 40;
		if (m == 9) return 35;
		if (m == 10) return 32;
		if (m == 11) return 30;
	...
	final zoomList:Array<Float> =
		#if !html5
		[
			0.0625, 0.125, 0.25, 0.5, 1, 2, 3, 4, 6, 8, 12, 16, 20, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576
		];
	...
	override function create() {
		...
		for (i in 0...(Note.ammo[_song.options.mania] * 2)) {
			var note:StrumNote = new StrumNote(GRID_SIZE * (i + 0.5), strumLine.y, i % Note.ammo[_song.options.mania], 0);
	...
	function addOptionsUI():Void {
		...
		if (skin == null)
			skin = 'NOTE_assets';
		noteSkinInputText = new FlxUIInputText(player2DropDown.x, player2DropDown.y + 90, 150, skin, 17);
		noteSkinInputText.focusGained = () -> FlxG.stage.window.textInputEnabled = true;
		blockPressWhileTypingOn.push(noteSkinInputText);

		noteSplashesInputText = new FlxUIInputText(noteSkinInputText.x + 155, noteSkinInputText.y /* + 35*/, 150, _song.assets.splashSkin, 17);
	...
	public function reloadNote() {
		...
		if (PlayState.isPixelStage) {
			...
			for (i in 0...mania + 1) {
				...
				animation.addByPrefix('pressed$i', Note.keysShit.get(PlayState.mania).get('letters')[i] + ' press', 48, false);
				animation.addByPrefix('confirm$i', Note.keysShit.get(PlayState.mania).get('letters')[i] + ' confirm', 48, false);
			}
		}
